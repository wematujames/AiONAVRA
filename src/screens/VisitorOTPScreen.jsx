...
const SignInScreen = ({ navigation }) => {
  const [signUp, setSignUp] = useState({ otpCode: "" });

  const authContext = useContext(AuthContext);
  const { visitorSignIn2Fa, state } = authContext;
  ...
  const onSubmit = async () => {
    await visitorSignIn2Fa(signUp.otpCode);
  };
      ...
      <BlurView intensity={10} style={styles.container}>
        <View style={styles.formContainer}>
          ...
          <TextInput
            ...
            onChangeText={(val) => onInputChange("otpCode", val)}
            autoCapitalize="none"
            autoCorrect={false}
            style={styles.input}
          />
